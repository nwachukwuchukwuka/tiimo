...
// const AddTaskSheet = forwardRef<BottomSheetModal, AddTaskSheetProps>(({ onAddTask }, ref) => {
...
//                         {/* More Options */}
//                         <TouchableOpacity className="bg-gray-100 w-8 h-8 rounded-xl items-center justify-center">
...
export interface AddTaskSheetProps {
    onAddTask: (title: string, priority: 'HIGH' | 'MEDIUM' | 'LOW' | 'TODO', duration: string) => void;
    // 1. New Prop to trigger the detailed modal
...
const AddTaskSheet = forwardRef<BottomSheetModal, AddTaskSheetProps>(({ onAddTask, onOpenDetails }, ref) => {
    ...
    const handleEllipsisClick = () => {
        Keyboard.dismiss();
        (ref as any)?.current?.dismiss();
        // 2. Trigger parent callback with current text
        onOpenDetails(text);
        setText('');
    };
